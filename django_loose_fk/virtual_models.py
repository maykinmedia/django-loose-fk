...
class ProxyMixin:
    def __init__(self, url: str, initial_data: dict, *args, **kwargs):
        self._loose_fk_data: dict[str, str | list[DictOrUrl]] = {"url": url}
...
def virtual_model_factory(model: ModelBase, loader) -> VirtualModelBase:
    ...
    try:
        ...
        assert isinstance(existing_model, VirtualModelBase)
...
class BaseHandler:
    ...
    def __set__(self, instance: models.Model, value: list[DictOrUrl]):
        assert isinstance(instance, ProxyMixin)
...
class FKHandler(BaseHandler):
    def __get__(self, instance, cls=None) -> models.Model | None:
        assert isinstance(instance, ProxyMixin)
