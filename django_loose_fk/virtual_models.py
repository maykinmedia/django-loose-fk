...
class ProxyMixin:
    def __init__(self, url: str, initial_data: dict, *args, **kwargs):
        self._loose_fk_data: Dict[str, Union[str, List[DictOrUrl]]] = {"url": url}
...
def virtual_model_factory(model: ModelBase, loader) -> VirtualModelBase:
    ...
    assert app_config is not None, f"Could not find app config for {model.__module__}"

    try:
        existing_model = apps.get_model(app_config.label, name)
    except LookupError:
        pass
    else:
        return cast(VirtualModelBase, existing_model)
...
class BaseHandler:
    ...
    def __set__(self, instance: models.Model, value: List[DictOrUrl]):
        cast(ProxyMixin, instance)._loose_fk_data[self.field_name] = value


class M2MHandler(BaseHandler):
    def __get__(self, instance, cls=None) -> QueryList:
        raw_data = cast(ProxyMixin, instance)._loose_fk_data.get(self.field_name, [])
...
class FKHandler(BaseHandler):
    def __get__(self, instance, cls=None) -> Optional[models.Model]:
        raw_data = cast(ProxyMixin, instance)._loose_fk_data.get(self.field_name, None)
        if raw_data is None:
            return None
